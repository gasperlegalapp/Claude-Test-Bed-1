...
export function TaskCard({ task }: Props) {
  ...
  return (
    <div className="card-base text-left w-full">
      {/* Case label */}
      {parentCase && (
      ...
      {task.notes && (
        <div className="flex items-start gap-1 mb-2">
          <StickyNote size={10} className="text-gray-600 mt-0.5 flex-shrink-0" />
      ...
      <div className="flex items-center justify-between gap-1 mt-2 pt-2 border-t border-white/5">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          {dueDateStr && (
            <div className={`flex items-center gap-1 text-[10px] flex-shrink-0 ${isOverdue ? 'text-red-400' : 'text-gray-500'}`}>
              <Calendar size={10} />
